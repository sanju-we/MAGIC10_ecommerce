...
const getTransactionDetails = async (req, res) => {
  try {
    ...
    if (transaction.orders && transaction.orders.length > 0) {
      for (let i = 0; i < transaction.orders.length; i++) {
        const orderDetails = await Order.findOne({ _id:new mongoose.Types.ObjectId(transaction.orders[i].orderId)})
        console.log('order.orderDetails:',transaction.orders[i].orderId)
        transaction.orders[i].orderDetails = orderDetails
      }
    }
